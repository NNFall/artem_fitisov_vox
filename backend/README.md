...
Бот доступен только ID из `TELEGRAM_ADMIN_CHAT_ID`/`TELEGRAM_ADMIN_IDS`. Админ отправляет CSV/TSV/XLSX с обязательной колонкой `phone`; backend создает кампанию в статусе `paused`. Запуск: `/run ID`, пауза: `/pause ID`.
